...
def get_column_names(decomp_levels, n_mfcc = 10):
...
def get_dataset(target_rate, n_samples = 10, n_levels = 10, n_mfcc = 10):
    random.seed(20)
    ...
    for i in range(0,n_samples):
        ...
        print(f'Processing (DWT): sample number {i} of {n_samples}')
        ...
        dwt_c = get_n_best_levels(y_c, n_levels)
        dwt_nc = get_n_best_levels(y_nc, n_levels)

        row_c = pd.DataFrame()
        row_nc = pd.DataFrame()
        print(f'Processing / feature extract: sample number {i} of {n_samples}')
        for (idx, (cD_c, cD_nc)) in enumerate(zip(dwt_c , dwt_nc)):
            if idx == 0:
                row_c = pd.DataFrame()
                row_c['name'] = [filename_c]
                row_c = row_c['name'].apply(get_features, data = cD_c , sr = sr_c, label = 'crackle', n_mfcc = n_mfcc)
                
                row_nc = pd.DataFrame()
                row_nc['name'] = [filename_nc]
                row_nc = row_nc['name'].apply(get_features, data = cD_nc , sr = sr_nc, label = 'no-crackle', n_mfcc = n_mfcc)
                continue 
            
            row_c_new = pd.DataFrame()
            row_c_new['name'] = [filename_c]
            row_c_new = row_c_new['name'].apply(get_features, data = cD_c , sr = sr_c, label = 'crackle', n_mfcc = n_mfcc)
            row_c = pd.merge(row_c, row_c_new ,on=[98,99])
            row_c['99'] = 'crackle'
            row_c['98'] = filename_c
            
            row_nc_new = pd.DataFrame()
            row_nc_new['name'] = [filename_nc]
            row_nc_new = row_nc_new['name'].apply(get_features, data = cD_nc , sr = sr_nc, label = 'no-crackle', n_mfcc = n_mfcc)
            row_nc = pd.merge(row_nc, row_nc_new, on=[98,99])
            row_nc['99'] = 'no-crackle'
            row_nc['98'] = filename_nc
    ...
    df.columns = get_column_names(n_levels , n_mfcc = n_mfcc)
...
def main():
    start = time.time()
    n_mfcc = 10
    n_samples = 2000 # This used 2012 seconds for all features
    df =  get_dataset(target_rate, n_samples = n_samples, n_levels = 10, n_mfcc = n_mfcc)
    print(df.shape)
    df.to_csv(f'features/new_features/DWT_complete_{n_samples}_samples.csv')
